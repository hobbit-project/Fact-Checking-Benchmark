...
public class DataGenerator extends AbstractDataGenerator {
    ...
    private List<String> datasetPaths = new ArrayList<String>();
    private boolean readOldFormat = false;
    ...
    public void init() throws Exception {
        ...
        String datasetUri = System.getenv(BenchmarkConstants.ENV_FACTBENCH_DATA_SET);
        if (datasetUri.startsWith("http://dice.cs.uni-paderborn.de/factcheck-benchmark/")) {
            if (datasetUri.equals("http://dice.cs.uni-paderborn.de/factcheck-benchmark/SWC2019-task2-train")) {
                readOldFormat = false;
                datasetPaths.add(DATA_GENERATOR_DIR + "SWC_2019_Train.nt");
            } else {
                readOldFormat = true;
                String pathParts[] = datasetUri.substring(52).split("/");
                if (pathParts.length > 1) {
                    datasetPaths.add(DATA_GENERATOR_DIR + pathParts[0] + File.separator + "wrong" + File.separator
                            + pathParts[1] + File.separator);
                } else {
                    datasetPaths.add(DATA_GENERATOR_DIR + pathParts[0] + File.separator + "wrong" + File.separator);
                }
                datasetPaths.add(DATA_GENERATOR_DIR + pathParts[0] + File.separator + "correct" + File.separator);
                logger.debug("FactBench directory {}", datasetPaths);
            }
        } else {
            throw new IllegalArgumentException("Got an unknown dataset URI " + datasetUri);
        }
    }
    ...
    protected void generateData() throws Exception {
        ...
        // id of this data generator [getGeneratorId()] and the number of all data
        // generators [getNumberOfGenerators()]
        // running in parallel.
        logger.debug("generateData()");
        for (String path : datasetPaths) {
            if (readOldFormat) {
                logger.info("Walking across {}", datasetPaths);
                sendDataFromDirectory(Paths.get(path));
            } else {
                sendDataFromFile(Paths.get(path));
            }
        }
    }
    ...
    public String returnStatementGraph(Resource statement, Model model) {
        ...
        Statement subjectLabel = model.listStatements(subject.getObject().asResource(), RDFS.label, (RDFNode) null)
                .nextStatement();
        Statement objectLabel = model.listStatements(object.getObject().asResource(), RDFS.label, (RDFNode) null)
                .nextStatement();

        Statement truthValue = model.listStatements(statement,
        ...
        String data = String.format(
                "%s:*:" + "<%s> " + "<%s> " + "<%s> " + ".\n" + "<%s> " + "<%s> " + "<%s> " + ".\n" + "<%s> " + "<%s> "
                        + "<%s> " + ".\n" + "<%s> " + "<%s> " + "<%s> " + ".\n" + "<%s> " + "<%s> " + "\"%s\"@en "
                        + ".\n" + "<%s> " + "<%s> " + "\"%s\"@en " + ".",
                truth, statement.toString(), RDF.type.toString(), RDF.Statement.toString(), subject.getSubject(),
                subject.getPredicate(), subject.getObject(), object.getSubject(), object.getPredicate(),
                object.getObject(), property.getSubject(), property.getPredicate(), property.getObject(),
    ...
    private String setFileIdentifier(String rootDir, String directory) {
        directory = directory.replace(rootDir, "").replace(".ttl", "").replace("\\", "-");
